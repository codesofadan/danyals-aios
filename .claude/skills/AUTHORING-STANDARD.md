...
## 0. Concrete wiring (read before §1)

> **Wave 7:** these are now **project skills** under `.claude/skills/` (auto-discovered from
> the repo root, no plugin manifest / marketplace), NOT the old `aios-seo` plugin. The shared
> client + rubric live in `.claude/skills/_shared/`. Anywhere older notes say `${CLAUDE_PLUGIN_ROOT}/…`,
> read `${CLAUDE_PROJECT_DIR}/.claude/skills/_shared/…`. `${CLAUDE_PROJECT_DIR}` in `allowed-tools`
> needs Claude Code v2.1.196+.

Two facts every skill depends on:

- **One shared backend client**, not per-skill curl: `_shared/aios_client.py`. Skills invoke it
  as `python ${CLAUDE_PROJECT_DIR}/.claude/skills/_shared/aios_client.py <subcommand>`.
  `${CLAUDE_PROJECT_DIR}` resolves to the repo root regardless of cwd.
...
- Skills call the backend through the **one shared client**, not ad-hoc curl, so the call
