...
# --- Autonomously-produced reports (the Reports library) ----------------------
# A downloadable report generated by a scheduled job (the monthly per-client SEO
# summary). ``id`` is the ledger row uuid used both as a React key and the download
# handle; the internal ``client_id`` never leaks (``client`` is the snapshot).


class GeneratedReportResponse(BaseModel):
    """One produced report card + download handle (from ``scheduled_job_runs``)."""
    ...
    period: str
    headline: str
    when: str = Field(description="relative created_at, e.g. '2d ago'")

    @classmethod
    def from_row(cls, row: dict[str, Any]) -> GeneratedReportResponse:
        report = row.get("report")
        headline = str(report.get("headline", "")) if isinstance(report, dict) else ""
        return cls(
            id=str(row["id"]),
            client=row.get("client_name", ""),
            ...
            headline=headline,
            when=relative_ago(row.get("created_at"), empty="just now"),
        )
