...
//
// `points.length === 0` was the ONLY branch, and a failed read produces exactly
// zero points — so an outage drew "No clients yet", a flat statement that the
// agency has never signed anyone. QueryGuard owns loading and failure; the
// EmptyState still owns a genuinely empty directory.
export default function ClientGrowth() {
  ...
  return (
    <section className="card">
      ...
      <QueryGuard queries={[clientsQ]} label="client growth" minHeight={130}>
        {points.length === 0 ? (
          <EmptyState
            icon="show_chart"
