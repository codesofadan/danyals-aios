...
| PM-3 | A client can never reach a staff route; staff routes require a permission no client holds | **PARTIAL — was marked "CONFIRMED — enforced + tested" and was false (measured 2026-08-24).** The second clause is the one that misled: a staff route did *not* require a permission, it required only authentication. `/rbac/{features,permissions,roles,templates}` and `/cost/pricing` returned the agency's access model and supplier unit prices to any signed-in principal, a portal client included, and served them from **in-process constants** so no RLS policy stood behind them. Closed by `require_staff()` with a negative test per boundary (D-19). **10 route handlers still carry `CurrentUserDep` alone** (AST-measured, listed in D-19), and each is now classified on **evidence**: five `clients.*` plus `activity.list_activity` and `tiers.list_tier_clients` are RLS-bounded — **measured 2026-08-24** against a built PostgreSQL 16 with all 85 migrations applied and the app's own identity mechanism reproduced, where every table they read returned **zero rows** to a portal client and all rows to staff; `tiers.list_tiers` / `list_feature_areas` are client-readable by decision; `auth.logout` is correctly open. The measurement also found that `cost.get_dial` was **not** RLS-bounded despite reading an RLS-protected table — it merges DB rows with an in-process catalogue and returned 18 items regardless — so the whole `cost.*` read surface is now `require_staff()`. `/tiers` is deliberately client-readable (D-19). |
