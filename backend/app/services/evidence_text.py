...
MIRROR of ``danyals-audit-system/audit_engine/evidence_text.py``. The audit
engine is a separate product with its own interpreter, invoked as a subprocess
and never imported here, so the two cannot share a module. They share a SPEC
instead, and the test suite on each side runs the same cases - including a
sweep over evidence the engine has actually emitted. Change one, change both.
