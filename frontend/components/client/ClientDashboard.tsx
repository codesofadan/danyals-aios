...
// GET /portal/dashboard) over a grid of report/graph cards. Every card the admin
// granted shows its live data immediately; cards that were never granted stay
// locked (an upsell to request access from their account manager).
export default function ClientDashboard() {
  const { grants, isGranted, reportViz, latestScore, latestAuditWhen, totalAudits } = useClient();

  const total = dashboardReports.length;
  const grantedCount = grants.size;

  // Honest header copy for every state: no graphs shared at all is NOT "all live".
  const note =
    grantedCount === 0
      ? "No graphs yet. Ask your account manager to share reports."
      : "Every report shared with you is showing";
    ...
    <div className="tw cl">
      <ClientHeader
        focus={
          <>
            ...
            <span className="cl-focus-v">{grantedCount} of {total} graphs shared</span>
      ...
      <div className="cl-stats">
        ...
        <div className="cl-stat-card">
          ...
          <div className="cl-stat-sub">
            {grantedCount === total ? "All reports shared" : `${total - grantedCount} not in your plan`}
          </div>
        </div>
      </div>
