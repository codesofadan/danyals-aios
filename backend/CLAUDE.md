...
- P4-6: `tests/integration/test_portal_isolation.py` (auto-skips without Supabase) provisions 2 client tenants + 1 staff and proves, **via each client's own JWT against PostgREST**, that base tables return 0 rows and `mrr/cost/error/*_path` are unreachable, the view shows only own audits, a Free run pins `client_id`, `delivery_tier='free'` blocks Paid, staff still see all, and clients are absent from the roster. **Verified live against Supabase (2026-07-14): passes.**
...
- P5-5: `tests/integration/test_tasks_flow.py` (auto-skips without Supabase) proves, **via each principal's own JWT hitting PostgREST directly**, that a non-lead status→done PATCH is REJECTED by the trigger (skip-review jump AND review self-sign-off), a non-status column edit is REJECTED, a lead CAN sign off review→done and reject review→in_progress, an assignee must be staff (reassign→client uid rejected), my-queue scoping works, and a portal client is fully excluded. **Verified live against Supabase (2026-07-14): passes.**
