...
**Who runs it.** Any provisioned staff (`view_reports`) - the read endpoints. Creating a fresh run (the optional route-out to `/aios-audit`) needs `run_audits`. If the caller lacks `view_reports` the reads 403 - report it and STOP.
...
- [ ] Step 5: If no suitable audit exists -> route to /aios-audit (do NOT silently run one)
```
...
5. **No audit? Route, do not spend.** If nothing suitable exists, tell the operator to run one via `/aios-audit` (technical is Free-tier). Do NOT create a run from this interpret skill unless the operator explicitly asks.
...
## Decision points
- If `$target` resolves to no `done` audit with `technical` -> STOP and route to `/aios-audit` (Free-tier technical run); do not fabricate findings.
...
- Running a new audit from here to "get fresh data" -> route to `/aios-audit`; this skill interprets, it does not own the run/spend.
