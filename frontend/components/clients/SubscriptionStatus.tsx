...
//
// `clients.length === 0` was the ONLY branch: on first paint and on every failed
// read it said "No clients yet" and printed $0 MRR — an empty book, asserted while
// the request was still in flight or already dead. QueryGuard owns the loading and
// failure halves; the EmptyState below still owns the genuinely-empty directory,
// because no clients IS data.
export default function SubscriptionStatus() {
  ...
  return (
    <section className="card">
      ...
      <QueryGuard queries={[clientsQ]} label="subscriptions" minHeight={148}>
        {clients.length === 0 ? (
          <EmptyState
            icon="donut_small"
