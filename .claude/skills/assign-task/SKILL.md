...
Rubric enforced (reference, not inlined): the task state machine (backend invariant, Part 5 - the `tasks_guard_update` trigger owns the lifecycle; the review gate is lead-only). Shared wiring + roles: `${CLAUDE_PROJECT_DIR}/.claude/skills/_shared/reference/`.
