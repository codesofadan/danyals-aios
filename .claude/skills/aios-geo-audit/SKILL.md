...
- [ ] Step 5: If no suitable geo audit -> route to /aios-audit (Paid; confirm spend first)
```
...
5. **No geo audit? Route, confirm the spend.** If none exists, a GEO run needs the **Paid** tier via `/aios-audit` (spends metered budget, cost-gated). Do NOT create a Paid run from this interpret skill.
...
## Decision points
- If `$target` resolves to no `done` audit with `geo` -> STOP and route to `/aios-audit` (Paid tier, spends, cost-gated); do not fabricate AI-visibility findings.
