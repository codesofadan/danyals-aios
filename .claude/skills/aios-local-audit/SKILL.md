...
**Who runs it.** Any provisioned staff (`view_reports`) - the reads. A fresh run (route-out to `/aios-audit`) needs `run_audits` AND the Paid tier. If the caller lacks `view_reports` the reads 403 - report it and STOP.
...
- [ ] Step 5: If no suitable local audit -> route to /aios-audit (Paid; confirm spend first)
```
...
5. **No local audit? Route, confirm the spend.** If none exists, tell the operator a local run needs the **Paid** tier via `/aios-audit` (spends metered budget, cost-gated). Do NOT create a Paid run from this interpret skill.
...
## Decision points
- If `$target` resolves to no `done` audit with `local` -> STOP and route to `/aios-audit` (Paid tier, spends, cost-gated); do not fabricate a GBP/NAP picture.
...
- Silently triggering a Paid local run to "get the data" -> it spends; route to `/aios-audit` and let the operator confirm.
